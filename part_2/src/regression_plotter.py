...
class RegressionPlotter():
    def __init__(self, model: MLModel) -> None:
        if not isinstance(model, MLModel):
            raise TypeError(
                f"model: expected MLModel or its subclass, got {type(model)}")
        self._model = model

    def _plot_single_feature(self, X: np.ndarray, y: np.ndarray,
                             axis: plt.Axes, feature_name: str = "Feature",
                             target_name: str = "Target") -> Axes:
        axis.scatter(X, y)
    ...
    def _plot_multiple_features(self, X: np.ndarray, y: np.ndarray,
        ...
        for i in range(X.shape[1]):
            ...
            sub_plotter._plot_single_feature(
                X[:, i], y, ax[i], feature_names[i], target_name)
        plt.show()

    def _plot_two_features(self, X: np.ndarray, y: np.ndarray,
        ...
        y = self._model.predict(
            np.c_[x1.ravel(), x2.ravel()]).reshape(x1.shape)
        ...
        if X.shape[0] != y.shape[0]:
            raise ValueError(
                f"X and y must have the same number of rows. \
                    + Found {X.shape[0]} rows in X and {y.shape[0]} \
                        + rows in y.")
        # needed to change this in the solution because parameters array \
            # includes both weights and intercept
        model_params = self._model.parameters
        if model_params is None:
            raise ValueError("Model parameters are not set.")
        if (data_features := X.shape[1]) != (len(model_params) - 1):
            raise ValueError(
                f"Features mismatch between the data passed and the model. \
                    + Found {data_features} features in data and \
                        + {len(model_params) - 1} weights in model")

    def plot(self, X: np.ndarray, y: np.ndarray,
             plot_3d_if_2_covariates: bool = True) -> None:
